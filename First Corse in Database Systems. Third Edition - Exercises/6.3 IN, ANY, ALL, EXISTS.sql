...
-- 6.3.8
-- write sql statement that will generate information about all products in one table

SELECT productpcnotebook.*, pr.color, pr.type, pr.price as printer_price
FROM 
(SELECT productpc.*, n.speed as notebook_speed, n.hardDrive as notebook_hardDrive, n.ram as notebook_ram, n.screen, n.price as notebook_price
FROM 
(SELECT p.*, pc.speed as pc_speed, pc.ram as pc_ram, pc.hardDrive as pc_hardDrive, pc.price as pc_price
FROM products p 
LEFT JOIN pc pc ON p.model = pc.model) productpc
LEFT JOIN notebook n ON productpc.model = n.model) productpcnotebook
LEFT JOIN printer pr ON productpcnotebook.model = pr.model;
